...
def main():
    ...
    # init nav_processing.py 的 Nav2Processing
    ...
    time.sleep(1)  # 節點剛啟動可能有延遲，給時間讓他確認publishers和subscribers已經建立
    #publish target label ="Pikachu"
    ros_communicator.publish_target_label("Pikachu")
    print("Target label published: Pikachu from pros_car")
    #nav2_processing.exploration_logic()
    ros_communicator.publish_init_pose()
    time.sleep(1.0)
    try:
        app.main()
    finally:
